...
var Game = function(args){
  ...
  function jsChessSerialise(repository) {
    var serialiseTurnLimit = 0;
    if(turnLimit !== Infinity) {
      serialiseTurnLimit = turnLimit;
    }
    return {
      id: id,
      idGenerator: repository.add(idGenerator),
      turnLimit: serialiseTurnLimit,
      player1: {
        name: player1.name,
...
Game.jsChessDeserialise = function(json, repository) {
  var player1 = {
    name: json.player1.name,
    army: repository.dereference(json.player1.army)
  };

  var player2 = {
    name: json.player2.name,
    army: repository.dereference(json.player2.army)
  };
